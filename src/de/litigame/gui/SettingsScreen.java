...
public class SettingsScreen extends Screen implements KeyPressedListener {
  ...
  public void render(Graphics2D g) {
    super.render(g);
    renderSoundBar(g, (int) (Game.window().getWidth() - settings.getWidth()) / 2,
        (int) (settings.getY() - settings.getHeight() / settings.getRows()));
  }

  private void renderSoundBar(Graphics2D g, int x, int y) {
    int margin = 11;

    BufferedImage bar = Resources.images().get("volume_bar_fill.png");
  ...
  @Override protected void initializeComponents() {
    ...
    settings = new Menu((Game.window().getWidth() - buttonImg.getWidth()) / 2d,
        (Game.window().getHeight() - buttonImg.getHeight() * items.length) / 2d,
